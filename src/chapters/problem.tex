...
De acuerdo con la tabla anterior, los cálculos de PA y PO pueden ser representados por las siguientes fórmulas, respectivamente.
...
\[
{PA} = \sum_{i=1}^{CC}x=x+f(pos) \cdot \textit{mul}
\]
...
\[
{PA} = \sum_{i=1}^{CC}x=x+f(pos) \cdot \left(\textit{mul} + \textit{delta}\right)
\]
